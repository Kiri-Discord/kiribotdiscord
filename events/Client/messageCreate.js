...
module.exports = async (client, message) => {
    ...
    if (message.channel.type === "DM") {
        ...
        setting = await client.dbFuncs.existingGuild(message.guild.id);
        if (!setting) {
            setting = await client.dbFuncs.createGuild(message.guild.id, true);
    ...
    if (
        ...
        return message.reply(
            `\`${prefix}${cmd}\` is under maintenance or is only avaliable for a limited number of people. You can try again later ${sed}`
        );
    ...
    if (!message.channel.nsfw && commandFile.conf.adult) {
        if (
            ...
            return message.channel.send(
                `you can only use this command in a NSFW channel please ${dead}`
            );
        } else {
    ...
    try {
        ...
        logger.log(
            ...
            } ran a command: ${prefix}${cmd} (cluster: ${client.cluster.id}, shard: ${message.guild.shardId})`
        );
    } catch (error) {
