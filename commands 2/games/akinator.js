...
class Game {
    ...
    async init() {
        ...
        else this.message.channel.send('the game was cancelled :pensive:');
    };
    ...
    async run() {
        ...
        this.msg.edit('', new MessageEmbed()
            ...
            .addFields({ name: 'Question:', value: this.aki.question }, { name: 'Answers:', value: `\n[0] ${this.aki.answers[0]}\n[1] ${this.aki.answers[1]}\n[2] ${this.aki.answers[2]}\n[3] ${this.aki.answers[3]}\n[4] ${this.aki.answers[4]}\n[5] Back` }, { name: 'Progress:', value: this.aki.progress }, ));
    ...
    async getResponse(filter) {
        let response;
        const responseAwait = await this.message.channel.awaitMessages({
            filter,
            max: 1,
            time: 30000,
            errors: ['time'],
