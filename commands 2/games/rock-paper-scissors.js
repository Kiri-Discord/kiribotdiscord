...
class Game {
    ...
    async run() {
        ...
        const p1Response = p1Message.channel.awaitMessages({
            filter,
            max: 1,
            time: 30000,
            errors: ['time'],
        ...
        const p2Response = p2Message.channel.awaitMessages({
            filter,
            max: 1,
            time: 30000,
            errors: ['time'],
    ...
    winChecks(values) {
        ...
        if (p1Choise === 'r' && p2Choise === 's') return this.win('p1');
        else if (p2Choise === 'r' && p1Choise === 's') return this.win('p2');
        else if (p2Choise === 'r' && p1Choise === 'r') return this.win('draw'); // Rock checks
        if (p1Choise === 'p' && p2Choise === 'r') return this.win('p1');
        else if (p2Choise === 'p' && p1Choise === 'r') return this.win('p2');
        else if (p2Choise === 'p' && p1Choise === 'p') return this.win('draw'); // Paper checks
        if (p1Choise === 's' && p2Choise === 'p') return this.win('p1');
        else if (p2Choise === 's' && p1Choise === 'p') return this.win('p2');
        else if (p2Choise === 's' && p1Choise === 's') return this.win('draw'); // Scissors checks
    }
