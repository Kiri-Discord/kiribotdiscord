...
exports.run = async(client, message, args, prefix) => {
        ...
        if (args[0].toLowerCase() === 'channel') {
            ...
            if (!channel.permissionsFor(message.guild.me).has(['EMBED_LINKS', 'SEND_MESSAGES'])) return message.reply({ embed: { color: "f3f3f3", description: "i don't have the perms to send greeting message to that channel! :pensive:\nplease allow the permission \`EMBED_LINKS\` **and** \`SEND_MESSAGES\` for me there before trying again." } });
    ...
    if (args[0].toLowerCase() === 'content') {
        ...
        const filter = res => {
            ...
            if (!types.includes(res.content)) {
                res.reply({ embed: { color: "f3f3f3", description: `\`${res.content}\` is not a valid type for the greeting message :pensive: you should choose again with either ${types.map(x => `\`${x}\``).join(" or ")}` } });
                return false;
            };
            return true;
        };
