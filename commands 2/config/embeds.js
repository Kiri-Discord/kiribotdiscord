...
exports.run = async(client, message, args, prefix) => {
    ...
    if (args[0].toLowerCase() === 'new') {
        ...
        if (storage.embeds.toObject().length > 100) return message.channel.send({ embed: { color: "RED", description: `your server has reached the 100 embeds created limit :pensive: please use \`${prefix}${exports.help.name}delete\` to delete unused embeds before creating another one.` } })
        let name = null;
        ...
        while (!targetEmbed.fields) {
            ...
            const check = lines.every(line => {
                ...
                if (varReplace.replaceText(fields[0], message.member, message.guild).length > 256) return false;
                if (varReplace.replaceText(fields[1], message.member, message.guild).length > 1024) return false;
                fieldsArray.push({
                    name: fields[0],
                    value: fields[1],
        ...
        await storage.save();
        return setupMessage.edit(embed2);
    } else if (args[0].toLowerCase() === 'send') {
        const id = args[1];
        if (!id) return message.reply({ embed: { color: "f3f3f3", description: `you haven't provided me an embed ID yet. to show all embeds that were created on this guild, do \`${prefix}embeds list\` first!` } });
        let embedsStorage = client.dbembeds;
        let storage = await embedsStorage.findOne({
            guildID: message.guild.id
        });
        ...
        if (!embed) return message.reply({ embed: { color: "f3f3f3", description: `there aren't any embed created on this server name \`${id}\` :pensive: to create a new embed, do \`${prefix}embeds new\`!` } });
        ...
        if (!channel.permissionsFor(message.guild.me).has('MANAGE_WEBHOOKS')) return message.reply({ embed: { color: "f3f3f3", description: "i don't have the perms to send embeds to that channel! :pensive:\nplease allow the permission \`EMBED_LINKS\` **and** \`SEND_MESSAGES\` for me there before trying again." } });
        return channel.send({ embed: varReplace.replaceEmbed(embed.embed, message.member, message.guild) })
        ...
        if (!storage.embeds.toObject().length) return message.reply({ embed: { color: "f3f3f3", description: `there aren't any embed created on this server yet :pensive: to create a new embed, do \`${prefix}embeds new\`!` } });
        const embed = storage.embeds.toObject().find(x => x._id === id);
        if (!embed) return message.reply({ embed: { color: "f3f3f3", description: `there aren't any embed created on this server name \`${id}\` :pensive: to create a new embed, do \`${prefix}embeds new\`!` } });
        storage.embeds.pull(embed);
        storage.markModified('embeds');
        await storage.save();
        return message.reply({ embed: { color: "f3f3f3", description: `☑️ that embed was deleted successfully` } });
    ...
    } else return message.channel.send({ embed: { color: "RED", description: `wrong sub command :pensive: \nall avaliable sub-command for setting up embeds are: \`new, send, list\`!` } })
};
