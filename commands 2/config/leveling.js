...
exports.run = async(client, message, args, prefix) => {
        ...
        if (args[0] === "on") {
            ...
            if (!args[1]) return message.channel.send({ embed: { color: "f3f3f3", description: `you haven't specified a destination for your leveling message yet!\nyou should choose one by using \`${prefix}leveling announce <there / #channel>\`!` } });
            if (args[1] === 'there') {
                ...
                await setting.save();
                return message.channel.send({ embed: { color: "f3f3f3", description: `☑️ levelings announcement will now be send in the same channel that the user is active!` } })
            } else {
                const channel = message.mentions.channels.first() || message.guild.channels.cache.get(args[1]);
                if (!channel) return message.channel.send({ embed: { color: "f3f3f3", description: 'i can\'t find that channel. pls mention a channel within this guild 😔' } });
                if (!channel.permissionsFor(message.guild.me).has(['EMBED_LINKS', 'SEND_MESSAGES'])) return message.reply({ embed: { color: "f3f3f3", description: "i don't have the perms to send leveling announcement to that channel! :pensive:\nplease allow the permission \`EMBED_LINKS\` **and** \`SEND_MESSAGES\` for me there before trying again." } });
                const setting = await client.dbguilds.findOne({
                    guildID: message.guild.id
                });
                ...
                await setting.save();
                return message.channel.send({ embed: { color: "f3f3f3", description: `☑️ levelings announcement will now be send in ${channel}!` } })
            };
        ...
        const filter = res => {
            ...
            if (!types.includes(res.content)) {
                res.reply({ embed: { color: "f3f3f3", description: `\`${res.content}\` is not a valid type for the goodbye message :pensive: you should choose again with either ${types.map(x => `\`${x}\``).join(" or ")}` } });
                return false;
            };
            return true;
        };
        ...
        if (type.content.toLowerCase() === 'plain') {
            ...
            if (!targetEmbed) return content.reply({ embed: { color: "f3f3f3", description: `there aren't any embed created on this server name \`${content.content}\` :pensive: to create a new embed, do \`${prefix}embeds new\`!` } });
            contentObject = {
                type: 'embed',
                content: targetEmbed
            };
        ...
        else return channel.send({ embed: varReplace.replaceEmbed(setting.levelings.content.content.embed, message.member, message.guild, { event: 'level', type: setting.responseType }, { level: 50, xp: 50 }) });
    } else {
        return message.channel.send({ embed: { color: "f3f3f3", description: `ℹ️ levelings is currently ${db.enableLevelings ? 'enabled' : 'disabled'} for our server. to setup the leveling system, use one of the following avaliable sub-command: \`off, on, content, announce, test\`!` } })
    }
}
