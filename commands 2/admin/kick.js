...
exports.run = async(client, message, args) => {
    ...
    if (!member.kickable) return message.reply({ embed: { color: "RED", description: 'this user can\'t be kicked. it\'s either because they are a mod/admin, or their highest role is equal or higher than mine 😔' } });

    if (message.member.roles.highest.position <= member.roles.highest.position) return message.reply({ embed: { color: "RED", description: 'you cannot kick someone with a higher or equal role!' } });
