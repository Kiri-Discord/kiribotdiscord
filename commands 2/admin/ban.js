...
exports.run = async(client, message, args) => {
    ...
    if (!member.bannable) return message.reply({ embed: { color: "RED", description: 'this user can\'t be banned. it\'s either because they are a mod/admin, or their highest role is equal or higher than mine 😔' } });

    if (message.member.roles.highest.position <= member.roles.highest.position) return message.reply({ embed: { color: "RED", description: 'you cannot ban someone with a higher role than you!' } });
