...
exports.run = async (client, message, args, prefix) => {
    ...
    if (args[0].toLowerCase() === "new") {
        ...
        `);
        await client.dbFuncs.saveEmbed(message.guild.id, targetEmbed, name, message.author.id);
            if (!setupMessage || !setupMessage.editable) {
                return message.channel.send({
                    embeds: [embed2],
                });
        ...
        const res = await client.dbFuncs.deleteEmbed(message.guild.id, id);
        if (!res)
            return message.reply({
                embeds: [
                    {
        ...
        const array = storage.embeds.map((each, index) => {
            const author = message.guild.members.cache.get(each.creator);
            return `\`${index + 1}\` - ${each._id} [${
