...
exports.run = async(client, message, args, prefix) => {
        ...
        if (args[0] === "on") {
            ...
            if (args[1] === 'there') {
                ...
                if (!channel.permissionsFor(message.guild.me).has(['EMBED_LINKS', 'SEND_MESSAGES'])) return message.inlineReply({ embed: { color: "f3f3f3", description: "i don't have the perms to send leveling announcement to that channel! :pensive:\nplease allow the permission \`EMBED_LINKS\` **and** \`SEND_MESSAGES\` for me there before trying again." } });
                const setting = await client.dbguilds.findOne({
                    guildID: message.guild.id
                });
