...
exports.run = async (client, message, args) => {
    ...
    if (!member) {
      return message.inlineReply(`you can't just poke at the air ${sedEmoji} please mention somebody to poke pls`)
    };
    const target = member.user;

    if (target.id === client.user.id) return message.inlineReply("you don't know how bad it will be, do you?")
    if (target.bot) return message.inlineReply("you can't poke that bot, sorry :(")
