...
exports.run = async(client, message, args, prefix) => {
        ...
        if (!channel) return message.reply({ embed: { color: "RED", description: '⚠️ you are not in a voice channel!' } });

        if (!channel.joinable) return message.reply({ embed: { color: "RED", description: "i can't join the voice channel where you are in. can you check my permission?" } });

        if (serverQueue && channel !== message.guild.me.voice.channel) {
            const voicechannel = serverQueue.channel;
            return message.reply({ embed: { color: "RED", description: `i have already been playing music to someone in your server! join \`#${voicechannel}\` to listen :smiley:` } });
        };
        if (!args.length) return message.reply({ embed: { color: "RED", description: `you must to provide me something to play! use \`${prefix}help play\` to learn more :wink:` } });
    ...
    if (!serverQueue) {
        ...
        try {
            ...
            return message.channel.send({ embed: { color: "RED", description: `:x: there was an error when i tried to join your voice channel` } }).catch(err => logger.log('error', err));
        }
    };
}
