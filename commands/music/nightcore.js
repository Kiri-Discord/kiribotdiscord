...
exports.run = async(client, message, args, prefix) => {
    ...
    if (queue.textChannel.id !== message.channel.id && !client.deletedChannels.has(queue.textChannel)) queue.textChannel.send({ embeds: [{ color: "#bee7f7", description: `${message.author} applied nightcore to the current queue ${sayoriEmoji}` }], content: `to reset all effect, use \`${prefix}reset\`` });
