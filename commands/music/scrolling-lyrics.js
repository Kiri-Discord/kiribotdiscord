...
exports.run = async(client, message, args, prefix) => {
    ...
    if (args[0] === "on") {
        if (!serverQueue) return message.channel.send({ embeds: [{ color: "#bee7f7", description: `❌ there isn't any queue to turn it on :pensive:` }] });
    ...
    if (args[0] === "lang") {
        if (!serverQueue) return message.channel.send({ embeds: [{ color: "#bee7f7", description: `❌ there isn't any queue to set the language for :pensive:` }] });
        const query = args[1];
    ...
    if (args[0] === 'set') {
        ...
        await Guild.findOneAndUpdate({
            ...
            .catch(err => logger.log('error', err));
        return message.channel.send(({ embeds: [{ color: "#bee7f7", description: `☑️ the scrolling lyric channel has been set to ${channel}!\n\ndo \`${prefix}scrolling-lyrics on\` to enable it :wink:` }] }));
    }
