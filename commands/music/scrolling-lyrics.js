...
exports.run = async(client, message, args, prefix) => {
    ...
    if (args[0] === "off") {
        ...
        return message.channel.send({ embed: { color: "f3f3f3", description: `❌ scrolling lyric has been disabled` } });
    };
    if (args[0] === "on") {
        if (!serverQueue) return message.channel.send({ embed: { color: "f3f3f3", description: `❌ there isn't any queue to turn it on :pensive:` } });
        if (!serverQueue.karaoke.channel) return message.channel.send({ embed: { color: "f3f3f3", description: `❌ the scrolling lyrics channel haven't been set yet. do \`${prefix}scrolling-lyrics set #yourchannel\` to set it first!` } });
        ...
        return message.channel.send(({ embed: { color: "f3f3f3", description: `☑️ scrolling lyric is turned on in ${serverQueue.karaoke.channel}!` } }));
