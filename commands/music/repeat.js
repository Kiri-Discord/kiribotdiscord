...
exports.run = async(client, message, args) => {
    ...
    if (queue.textChannel.id !== message.channel.id) message.channel.send({ embeds: [{ color: "#bee7f7", description: `repeat is turned ${queue.repeat ? "on" : "off"} for the current song 🔁` }] })
    queue.textChannel.send({ embeds: [{ color: "#bee7f7", description: `${message.author} turn repeat ${queue.repeat ? "on" : "off"} for the current song 🔁` }] }).catch(err => logger.log('error', err));
};
exports.help = {
    name: "repeat",
