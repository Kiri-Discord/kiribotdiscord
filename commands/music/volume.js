...
exports.run = async (client, message, args) => {
    ...
    if (!queue) return message.reply('i can\'t change the volume since there isn\'t anything in the queue :grimacing:');
    ...
    if (Number(args[0]) > 100 || Number(args[0]) < 0) return message.reply({embed: {color: "f3f3f3", description: `invaild number :pensive: the amount of volume must not lower than zero, and must not higher than 100% ❌`}});
    queue.volume = args[0];
