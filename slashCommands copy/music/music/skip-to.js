...
async function skip(queue, interaction, index, sentMessage, client) {
    ...
    queue.skip();
    const number = index - 1;
    if (queue.textChannel.id !== interaction.channel.id && !client.deletedChannels.has(queue.textChannel)) queue.textChannel.send({ embeds: [{ color: "#bee7f7", description: `${interaction.user} skipped ${number} songs ⏭` }] });
