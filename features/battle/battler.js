...
module.exports = class Battler {
		...
		const filter = res => {
			...
			if (res.author.id === this.user.id && choices.includes(choice)) {
				if ((choice === 'special' && !this.canSpecial) || (choice === 'cure' && !this.canHeal)) {
					channel.send('you don\'t have enough MP for that!').catch(() => null);
					return false;
				}
				if (choice === 'final' && !this.canFinal) {
					channel.send('you must have under 100 HP and over 50 MP. **final** can only be used once!').catch(() => null);
					return false;
				}
				return true;
			}
		...
		const turn = await channel.awaitMessages({
			filter,
			max: 1,
			time: 30000
		});
