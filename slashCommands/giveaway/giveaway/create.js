...
exports.run = async(client, interaction) => {
    ...
    async function ending(reason) {
        ...
        if (reason === 1) return interaction.channel.send(`:boom: the giveaway setup was cancelled due to inactivity :pensive:`);
    ...
    interaction.reply({ embeds: [{ color: "#bee7f7", description: `✅ the embed creation setup was started! please follow my guide ${duh}` }], ephemeral: true })
    ...
    while (!targetChannel) {
        ...
        const res = await askString(interaction.channel, filter, { time: 60000 });
        ...
        if (!channel) {
            ...
            embed
                ...
                .setDescription(stripIndents `
			:boom: that wasn't an existing channel in this server! mention a channel like this ${interaction.channel.toString()}
    ...
    while (!pingRole) {
        ...
        const role = interaction.guild.roles.cache.find(r => (r.name === res.content) || (r.id === res.content.replace(/[^\w\s]/gi, '')));
        if (!role) {
            ongoing = true;
            await deleteIfAble(res);
            embed
