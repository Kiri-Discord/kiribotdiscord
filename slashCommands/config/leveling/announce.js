exports.run = async(client, interaction, db) => {
    ...
    if (!channel.permissionsFor(interaction.guild.me).has(['EMBED_LINKS', 'SEND_MESSAGES'])) return interaction.reply({ embeds: [{ color: "#bee7f7", description: `i don't have the perms to send leveling announcement to ${channel}!\nplease allow the permission \`EMBED_LINKS\` **and** \`SEND_MESSAGES\` for me there before trying again please :pensive:` }], ephemeral: true });
    await interaction.deferReply();
    const setting = await client.dbguilds.findOne({
        guildID: interaction.guild.id
    });
