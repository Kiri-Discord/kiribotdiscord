...
exports.run = async(client, interaction) => {
    ...
    if (!data || !data.length) return interaction.editReply({ embeds: [{ color: "#bee7f7", description: `❌ seems like no one in your guild was cuddled yet :rolling_eyes: once someone is cuddled, it will show here!` }] });
    const emoji = {
        "1": ":crown:",
        "2": ":trident:",
        "3": ":trophy:",
    ...
    arrSplitted.forEach((item, index) => {
        const embed = new MessageEmbed()
            ...
            .setDescription(`you can cuddle others with \`/roleplay cuddle\`!`)
            .addField('\u200b', item.join('\n'))
