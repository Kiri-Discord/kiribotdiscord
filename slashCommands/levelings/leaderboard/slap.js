...
exports.run = async(client, interaction) => {
    ...
    if (!data || !data.length) return interaction.editReply({ embeds: [{ color: "#bee7f7", description: `❌ seems like no one in your guild has decided to slap yet :grinning: once someone is hugged, their slapped count will show here :pensive:` }] });
    const emoji = {
        "1": ":crown:",
        "2": ":trident:",
        "3": ":trophy:",
    ...
    arrSplitted.forEach((item, index) => {
        const embed = new MessageEmbed()
            ...
            .setDescription(`you can slap others with \`/roleplay pat\``)
            .addField('\u200b', item.join('\n'))
