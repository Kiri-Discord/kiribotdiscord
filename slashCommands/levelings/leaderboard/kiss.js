...
exports.run = async(client, interaction) => {
    ...
    if (!data || !data.length) return interaction.editReply({ embeds: [{ color: "#bee7f7", description: `❌ seems like no one in your guild was kissed yet :rolling_eyes: once someone is kissed, it will show here!` }] });
    const emoji = {
        "1": ":crown:",
        "2": ":trident:",
        "3": ":trophy:",
    ...
    arrSplitted.forEach((item, index) => {
        const embed = new MessageEmbed()
            ...
            .setDescription(`you can kiss others with \`/roleplay kiss\` :flushed:`)
            .addField('\u200b', item.join('\n'))
