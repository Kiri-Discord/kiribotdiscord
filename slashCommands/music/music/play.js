...
exports.run = async(client, interaction, internal, bulkAdd) => {
    ...
    if (serverQueue && !canModifyQueue(interaction.member)) {
        ...
        return interactionReply(interaction, { embeds: [{ color: "#bee7f7", description: `i have already been playing music in your server! join ${voicechannel} to listen :smiley:` }], ephemeral: true });
    };
    ...
    if (serverQueue) {
        ...
        if (interaction.channel.id !== serverQueue.textChannel.id && !client.deletedChannels.has(serverQueue.textChannel)) serverQueue.textChannel.send({ embeds: [embed] });
        if (client.deletedChannels.has(serverQueue.textChannel)) serverQueue.textChannel = interaction.channel;
