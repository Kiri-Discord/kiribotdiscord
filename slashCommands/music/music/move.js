...
exports.run = async(client, interaction) => {
    ...
    if (queue.textChannel.id !== interaction.channel.id && !client.deletedChannels.has(queue.textChannel)) queue.textChannel.send({ embeds: [{ color: "#bee7f7", description: `${interaction.user} moved **${song.info.title}** to position ${index} 🚚` }] });
