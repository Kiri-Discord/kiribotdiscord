...
exports.run = async(client, interaction) => {
    ...
    if (queue) {
        if (queue.pending) return interaction.reply({ embeds: [{ color: "#bee7f7", description: `:x: i'm still being connected to a queue! you can wait until i finish to disconnect :slight_smile: if this is taking too long, use \`/invite\` to visit my support server!` }], ephemeral: true });
    };
