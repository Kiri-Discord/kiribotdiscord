...
async function skip(queue, interaction, index) {
    ...
    if (queue.textChannel.id !== interaction.channel.id) queue.textChannel.send({ embeds: [{ color: "#bee7f7", description: `${interaction.user} skipped ${number} songs ⏭` }] });
    if (sentMessage) {
