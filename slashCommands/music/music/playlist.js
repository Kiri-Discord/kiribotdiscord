...
exports.run = async (client, interaction, internal, bulkAdd) => {
    ...
    if (!channel)
        return interactionReply(interaction, {
            embeds: [
                {
                    color: "#bee7f7",
                    description: "⚠️ you are not in a voice channel!",
                },
            ],
            ephemeral: true,
        });
    ...
    if (serverQueue && !canModifyQueue(interaction.member)) {
        ...
        return interactionReply(interaction, {
            embeds: [
                {
                    color: "#bee7f7",
                    description: `i have already been playing music in your server! join ${voicechannel} to listen :smiley:`,
                },
            ],
        });
    }
            ...
            embeds: [
                {
                    color: "#bee7f7",
                    description:
                        "i can't join or talk in the voice channel where you are in. can you check my permission?",
                },
            ],
            ephemeral: true,
        });
            ...
            embeds: [
                {
                    color: "#bee7f7",
                    description:
                        "there was a media error while i was initializing the queue! can you try again later? :pensive:",
                },
            ],
        });
    ...
    if (musicSettings && !internal) {
        ...
        const channel = interaction.guild.channels.cache.get(
            musicSettings.KaraokeChannelID
        );
        if (musicSettings.KaraokeChannelID && !serverQueue && channel) {
            const msg1 = await interaction.channel.send({
                embeds: [
                    {
                        description: `scrolling lyric mode was set to **ON** in the setting and all lyrics will be sent to ${channel}\ndo you want me to enable this to your queue, too? \`y/n\`\n\nyou only have to do this **ONCE** only for this queue :wink:`,
                        footer: {
                            text: `type 'no' or leave this for 10 second to bypass`,
                        },
                    },
                ],
            });
            ...
            if (verification) {
                const msg2 = await interaction.channel.send({
                    embeds: [
                        {
                            description: `nice! okay so what language do you want me to sing in for the upcoming queue?\nresponse in a valid language: for example \`English\` or \`Japanese\` to continue :arrow_right:`,
                            footer: {
                                text: "this confirmation will timeout in 10 second. type 'cancel' to cancel this confirmation.",
                            },
                        },
                    ],
                });
                const response = await verifyLanguage(
                    interaction.channel,
                    interaction.user,
                    { time: 10000 }
                );
                if (response.isVerify) {
                    ...
                    interaction.channel.send(
                        "you didn't answer anything! i will just play the song now..."
                    );
                }
            } else {
                msg1.delete();
    ...
    if (bulkAdd) {
        ...
        try {
            ...
            if (!newSongs || !newSongs.length)
                return interaction.editReply({
                    embeds: [
                        {
                            color: "RED",
                            description: `:x: no match were found`,
                        },
                    ],
                });
            playlistURL = url;
            ...
            return interaction.editReply({
                embeds: [
                    { color: "RED", description: `:x: no match were found` },
                ],
            });
        }
            ...
            if (!newSongs || !newSongs.length)
                return interaction.editReply({
                    embeds: [
                        {
                            color: "RED",
                            description: `:x: no match were found (SoundCloud tends to break things as i'm working on my end. try again later!)`,
                        },
                    ],
                });
            playlistURL = url;
        ...
        if (fields[1] === "episode" || fields[1] === "show") {
            return interaction.editReply({
                embeds: [
                    {
                        color: "#bee7f7",
                        description: `${logo} sorry, i don't support podcast link from Spotify :pensive:`,
                    },
                ],
            });
        }
                    ...
                    if (!newSongs || !newSongs.length)
                        return interaction.editReply({
                            embeds: [
                                {
                                    color: "RED",
                                    description: `:x: i failed when fetching the information for you... (YouTube tends to break things as i'm working on my end. try again later!)`,
                                },
                            ],
                        });
    ...
    let playlistEmbed = new MessageEmbed().setDescription(
        `✅ Added **${newSongs.length}** ${
            newSongs.length > 1
                ? `[tracks](${playlistURL || newSongs[0].info.uri})`
                : `[track](${playlistURL || newSongs[0].info.uri})`
        } to the queue [${interaction.user}]`
    );
    if (
        serverQueue &&
            ...
            return interaction
                .followUp({
                    embeds: [
                        {
                            color: "RED",
                            description: `:x: there was an error when i tried to join your voice channel!`,
                        },
                    ],
                })
