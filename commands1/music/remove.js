...
exports.run = async(client, message, args, prefix) => {
    ...
    if (pattern.test(arguments)) {
        ...
        if (queue.textChannel.id !== message.channel.id && !client.deletedChannels.has(queue.textChannel)) queue.textChannel.send({ embeds: [{ color: "#bee7f7", description: `${message.author} removed **${removed.map((song) => song.info.title).join("\n")}** from the queue ❌` }] });
        if (client.deletedChannels.has(queue.textChannel)) queue.textChannel = message.channel;
        return message.channel.send({ embeds: [{ color: "#bee7f7", description: `❌ you removed **${removed.map((song) => song.info.title).join("\n")}** from the queue` }] });
    } else if (!isNaN(args[0]) && songs.length <= 1 && args[0] > 1 && args[0] <= queue.songs.length) {
        if (queue.textChannel.id !== message.channel.id && !client.deletedChannels.has(queue.textChannel)) queue.textChannel.send({ embeds: [{ color: "#bee7f7", description: `${message.author} removed **${queue.songs.splice(args[0] - 1, 1)[0].info.title}** from the queue ❌` }] });
        if (client.deletedChannels.has(queue.textChannel)) queue.textChannel = message.channel;
        return message.channel.send({ embeds: [{ color: "#bee7f7", description: `❌ you removed **${queue.songs.splice(args[0] - 1, 1)[0].info.title}** from the queue ❌` }] });
    } else {
